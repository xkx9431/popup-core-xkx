...
export type Options = {
    ...
    function preventOverflow({ state, options, name }: ModifierArguments<Options>) {
        ...
        const mainAxis = getMainAxisFromPlacement(basePlacement); // @ts-ignore
        ...
        if (checkMainAxis) {
            ...
            ? mainAxis === 'y' // @ts-ignore
                ? arrowOffsetParent.clientTop || 0 // @ts-ignore
                : arrowOffsetParent.clientLeft || 0
            : 0;

            const offsetModifierValue = state.modifiersData.offset // @ts-ignore
        ...
        if (checkAltAxis) {
            ...
            const altSide = mainAxis === 'x' ? bottom : right; // @ts-ignore
            const offset = popperOffsets[altAxis];

            const min = offset + overflow[mainSide];
            const max = offset - overflow[altSide];

            const preventedOffset = within(min, offset, max); // @ts-ignore

            popperOffsets[altAxis] = preventedOffset; // @ts-ignore
